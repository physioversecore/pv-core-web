...
export function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-[90]">
      <button className="absolute inset-0 bg-forest/50" onClick={() => setOpen(false)} aria-label="Close cart" />
      <aside className="absolute right-0 top-0 h-full w-full max-w-md bg-cream border-l border-border flex flex-col">
        <header className="p-5 flex items-center justify-between border-b border-border">
          ...
          <button onClick={() => { setOpen(false); setCheckout(false); setOrderId(null); }} className="p-2 rounded-full hover:bg-sage"><X size={18} /></button>
        </header>
        ...
        {orderId ? (
          <div className="flex-1 grid place-items-center p-6 text-center">
            <div>
              ...
              <p className="text-slate text-sm mb-1">Order reference</p>
              <p className="font-mono text-pine font-semibold mb-5">{orderId}</p>
          ...
          <div className="flex-1 grid place-items-center text-center p-6">
            <div>
              <ShoppingBag size={48} className="mx-auto text-slate mb-3" />
              <p className="font-display text-xl mb-1">Cart is empty</p>
              <p className="text-slate text-sm mb-4">Browse our shop to get started.</p>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              {items.map((i) => (
                <div key={i.id + i.type} className="card-soft p-3 flex gap-3">
                  <div className="w-14 h-14 rounded-xl bg-sage grid place-items-center text-2xl shrink-0">{i.image ?? "📦"}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-2">
                      <div className="font-medium text-sm truncate">{i.name}</div>
                      <button onClick={() => remove(i.id)} className="text-slate hover:text-destructive"><Trash2 size={14} /></button>
                    </div>
                    <div className="text-xs text-slate capitalize">{i.type === "rent" ? "Rental" : i.type === "buy" ? "Purchase" : i.type}</div>
                    {i.type === "rent" && (
          ...
          <>
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div>
                <label className="text-xs font-medium text-slate">Delivery address</label>
              ...
              <div>
                <label className="text-xs font-medium text-slate">Payment method</label>
                <div className="grid gap-2 mt-1">
                  {[
                    ...
                    <label key={m.id} className={`p-3 rounded-xl border cursor-pointer ${payment === m.id ? "border-pine bg-sage" : "border-border bg-white"}`}>
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className={`flex justify-between text-sm ${bold ? "font-bold text-base text-forest" : "text-slate"}`}>
