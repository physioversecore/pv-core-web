...
export function CartDrawer() {
  ...
  const RENTAL_OPTIONS = [
    { label: t("cart.rentalOption1Day"), days: 1 },
    { label: t("cart.rentalOption1Week"), days: 7 },
    { label: t("cart.rentalOption2Weeks"), days: 14 },
    { label: t("cart.rentalOption1Month"), days: 30 },
  ];
  ...
  const placeOrder = () => {
    if (!address.trim()) return toast.error(t("cart.errorAddAddress"));
  ...
  return (
    <div className="fixed inset-0 z-[90]">
      <button className="absolute inset-0 bg-text/50" onClick={() => setOpen(false)} aria-label={t("common.close")} />
      <aside className="absolute right-0 top-0 h-full w-full max-w-md bg-background border-l border-border flex flex-col">
        <header className="p-5 flex items-center justify-between border-b border-border">
          <h3 className="font-display text-xl">{orderId ? t("cart.orderPlaced") : checkout ? t("cart.checkout") : t("cart.title")}</h3>
        ...
        {orderId ? (
          ...
          <div className="flex-1 grid place-items-center text-center p-6">
            <div>
              ...
              <p className="font-display text-xl mb-1">{t("cart.cartIsEmpty")}</p>
              <p className="text-text-light text-sm mb-4">{t("cart.emptyCartDesc")}</p>
              <button onClick={() => setOpen(false)} className="btn-outline">{t("common.browseShop")}</button>
            </div>
          </div>
        ) : !checkout ? (
          <>
              ...
              {items.map((i) => (
                <div key={i.id + i.type} className="card-soft p-3 flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-text-light capitalize">{i.type === "rent" ? t("cart.rental") : i.type === "buy" ? t("cart.purchase") : i.type}</div>
                    {i.type === "rent" && (
          ...
          <>
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div>
                <label className="text-xs font-medium text-text-light">{t("cart.deliveryAddress")}</label>
                <textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={3} className="w-full mt-1 px-3 py-2 rounded-xl border border-border bg-white" placeholder={t("cart.placeholderAddress")} />
              </div>
              <div>
                <label className="text-xs font-medium text-text-light">{t("cart.paymentMethod")}</label>
                <div className="grid gap-2 mt-1">
                  {[
                    { id: "esewa", label: t("cart.esewa") },
                    { id: "khalti", label: t("cart.khalti") },
                    { id: "cod", label: t("cart.cashOnDelivery") },
                  ].map((m) => (
