...
import { X, Eye, EyeOff, Stethoscope, HeartPulse, Loader2, ArrowLeft } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg max-h-[92vh] bg-background rounded-3xl border border-border shadow-2xl overflow-hidden">
        <button onClick={onClose} className="absolute right-4 top-4 z-10 p-2 rounded-full hover:bg-surface" aria-label={t("common.close")}>
          <X size={18} />
        </button>
        ...
        {!submitted && otpStep && (
          <div className="space-y-4">
            <button type="button" onClick={() => { setOtpStep(null); setOtpCode(""); }} className="text-xs text-secondary flex items-center gap-1">
        ...
        {!submitted && !otpStep && mode === "signup" && !signupRole && (
          <>
        ...
        {!submitted && !otpStep && mode === "signup" && signupRole === "patient" && (
          <form onSubmit={handlePatientSignup} className="space-y-3">
            ...
            <Field label={t("auth.labelPassword")} type="password" value={form.password ?? ""} onChange={(v) => set("password", v)} placeholder={t("auth.placeholderPassword")} />
            <div>
              <Field label={t("auth.labelConfirmPassword")} type="password" value={form.confirm ?? ""} onChange={(v) => set("confirm", v)} placeholder={t("auth.placeholderConfirm")} />
        ...
        {!submitted && !otpStep && mode === "signup" && signupRole === "therapist" && (
