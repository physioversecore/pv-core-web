...
export function StepPayment({
  ...
  const basePrice = therapist?.price ?? 0;
  const converted = currency ? basePrice * currency.rate : basePrice;
  ...
  const isNepalWallet = nepalWalletMethods.some((m) => m.id === selectedPaymentId);

  const isValid =
    selectedPaymentId &&
    (paymentType === "nepal"
      ? !isNepalWallet || (esewaMobile?.length ?? 0) >= 10
      : selectedPaymentId !== "card" ||
        (cardDetails.number.length >= 16 &&
          cardDetails.expiry.length >= 4 &&
          cardDetails.cvv.length >= 3 &&
          cardDetails.name.length > 0 &&
          (billingCountry?.length ?? 0) > 0));
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                setMethodOpen(false);
                onSelect(pt.key === "nepal" ? NEPAL_PAYMENTS[0] : INTERNATIONAL_PAYMENTS[0]);
              }}
              className={cn(
