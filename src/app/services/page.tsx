    ...
    >
            ...
            {SHOP.map((s, i) => (
              <Reveal key={s.title} delay={i * 80}>
                <div className="card-soft p-6 relative group hover:-translate-y-1 hover:shadow-[0_18px_38px_-18px_rgba(47,93,80,.45)] transition duration-300">
                  {s.live ? <span className="chip !bg-secondary !text-white absolute top-4 right-4">Live</span> : <span className="chip absolute top-4 right-4">Soon</span>}
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-5 lg:px-8 text-center">
          ...
          <p className="text-text-light mb-6">Talk to our care team — we&apos;ll match you to the right specialist.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/find" className="btn-primary">Find a therapist</Link>
            <Link href="/contact" className="inline-flex items-center gap-2 px-5 py-3 rounded-full font-semibold border border-secondary text-secondary hover:bg-secondary hover:text-white transition">Contact us</Link>
          </div>
        </div>
      </section>
    </PageShell>
