  ...
  return (
    ...
    >
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-5 lg:px-8 space-y-5 text-slate">
            ...
            {VALUES.map((v, i) => (
              <Reveal key={v.t} delay={i * 80}>
                <div className="card-soft p-6">
                  <div className="w-11 h-11 rounded-xl grid place-items-center mb-3 text-pine" style={{ background: "#D1E8DF" }}>{v.icon}</div>
                  <div className="font-display text-lg mb-1">{v.t}</div>
                  <p className="text-slate text-sm">{v.d}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-8 grid grid-cols-2 md:grid-cols-4 text-center">
          {[
            ...
            <div key={i} className={`py-4 ${i > 0 ? "md:border-l border-border" : ""}`}>
              <div className="font-display text-4xl text-pine">
                {i === 3 ? <>4.8<span className="text-amber">\u2605</span></> : <><CountUp to={s.to} /><span className="text-amber">{s.s}</span></>}
              </div>
              <div className="text-xs text-slate mt-2 font-mono uppercase tracking-widest">{i === 3 ? "Average rating" : s.l}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-5 lg:px-8 text-center">
          ...
          <p className="text-slate mb-6">We&apos;re always looking for verified physiotherapists and hospital partners.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/contact" className="btn-primary">Get in touch</Link>
            <Link href="/therapists" className="inline-flex items-center gap-2 px-5 py-3 rounded-full font-semibold border border-pine text-pine hover:bg-pine hover:text-white transition">Meet the therapists</Link>
          </div>
        </div>
      </section>
    </PageShell>
