...
function ServiceCard({ service, index, badge }: { service: ServiceData; index: number; badge?: string }) {
  const { t } = useLang();
          ...
          <article className="card-neo card-neo-hover overflow-hidden flex flex-col md:flex-row h-full group">
            <div className="md:w-1/2 p-6 md:p-10 flex flex-col justify-between order-2 md:order-1">
              ...
              <div className="mt-8">
                ...
                <h3 className="font-display font-extrabold uppercase text-2xl md:text-4xl tracking-tighter text-carbon mb-4">{service.name}</h3>
                <p className="text-text-light text-sm leading-relaxed max-w-md">{service.description}</p>
    ...
    <Reveal className="md:col-span-6 lg:col-span-4" delay={index * 80}>
      <Link href={`/services/${service.id}`} className="block h-full">
        <div className="card-neo card-neo-hover p-6 relative group h-full">
          ...
          <div className="w-12 h-12 rounded-xl grid place-items-center mb-3 text-carbon bg-volt border-2 border-carbon group-hover:scale-110 group-hover:rotate-6 transition duration-300"><Icon /></div>
    ...
    >
      <section className="py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <Reveal>
            ...
            <h2 className="text-4xl md:text-5xl font-display font-extrabold uppercase tracking-tighter mb-10 max-w-2xl">{t("services.clinicalTitle")}</h2>
          </Reveal>
      ...
      <section className="py-16 md:py-20 bg-moss grid-bg">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <Reveal>
            ...
            <h2 className="text-4xl md:text-5xl font-display font-extrabold uppercase text-paper-bright tracking-tighter mb-10 max-w-2xl">{t("services.shopTitle")}</h2>
          </Reveal>
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            {isLoading
              ? Array.from({ length: 6 }).map((_, i) => <CardSkeleton key={i} />)
              : shopServices.map((s, i) => <ServiceCard key={s.id} service={s} index={i} badge={t("services.soon")} />)}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-20 bg-paper">
        <div className="max-w-4xl mx-auto px-5 lg:px-8 text-center">
          <h2 className="font-display font-extrabold text-4xl uppercase tracking-tighter mb-2">{t("services.ctaTitle")}</h2>
          <p className="text-text-light mb-8">{t("services.ctaDesc")}</p>
            <Link href="/contact" className="btn-outline-ink uppercase text-sm">{t("services.ctaContact")} → </Link>
        </div>
      </section>
    </PageShell>
  );
