...
export default function Therapists() {
  ...
  return (
    <PageShell
      eyebrow={t("therapists.eyebrow")}
      title={t("therapists.title")}
      subtitle={t("therapists.subtitle")}
    >
      <section className="relative py-16 overflow-hidden text-background bg-background-dark">
        ...
        <div className="relative max-w-7xl mx-auto px-5 lg:px-8">
          <Reveal>
            <p className="eyebrow !text-primary mb-3">{t("therapists.featuredEyebrow")}</p>
            <h2 className="text-3xl font-display mb-10 max-w-2xl">{t("therapists.featuredTitle")}</h2>
          </Reveal>
          <div className="grid md:grid-cols-3 gap-6">
            {featured.map((th, i) => {
              const initials = th.name.replace("Dr. ", "").split(" ").map((s) => s[0]).slice(0, 2).join("");
              return (
                <Reveal key={th.id} delay={i * 120}>
                  <div
                  ...
                  >
                    <span className="absolute right-4 top-4 chip !bg-white/95 !text-secondary">{t("therapists.nmcVerified")}</span>
                    ...
                    <div className="flex items-center gap-1 text-xs relative z-10">
                      ...
                      <span className="font-semibold">{th.rating}</span>
                      <span className="text-white/70">({th.reviews} {t("therapists.reviews")})</span>
                    </div>
                    <div className="relative z-10">
                      <div className="font-display text-2xl">{th.name}</div>
                      <div className="text-sm text-white/80 mb-4">{th.specialty} · {th.city}</div>
                      <button onClick={() => handleBook(th)} className="btn-primary !py-1.5 !px-4 text-sm">{t("common.book")} · {npr(th.price)}</button>
                    </div>
                  </div>
                </Reveal>
              );
