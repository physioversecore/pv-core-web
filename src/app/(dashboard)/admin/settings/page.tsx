  ...
  return (
    <div>
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <Listed title="Supported cities" items={cities} setItems={setCities} input={newCity} setInput={setNewCity} />
        <Listed title="Supported specialties" items={specs} setItems={setSpecs} input={newSpec} setInput={setNewSpec} />

        <div className="card-soft p-5 lg:col-span-2">
          <p className="eyebrow mb-2">Welcome email template</p>
          ...
          <button onClick={() => toast.success("Template saved")} className="btn-pine mt-3 !py-1.5 !px-3 text-xs">Save template</button>
        </div>
      </div>
    </div>
  );
...
function Listed({ title, items, setItems, input, setInput }: { title: string; items: string[]; setItems: (v: string[]) => void; input: string; setInput: (v: string) => void }) {
  const add = () => { if (input.trim()) { setItems([...items, input.trim()]); setInput(""); toast.success("Added"); } };
  return (
    <div className="card-soft p-5">
      ...
      <div className="flex gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Add new…" className="flex-1 px-3 py-2 rounded-xl border border-border bg-white text-sm" />
        <button onClick={add} className="btn-pine !py-1.5 !px-3 text-xs">Add</button>
      </div>
    </div>
  );
}
