  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); toast.success("Profile saved"); }} className="card-soft p-6 max-w-2xl space-y-4">
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-sage grid place-items-center text-pine font-display text-xl">{f.name[0] ?? "T"}</div>
      ...
      <div>
        <label className="text-xs font-medium text-slate">Bio</label>
      ...
      <div className="p-3 rounded-xl bg-sage/60 text-xs text-slate">NMC License: <span className="font-mono text-pine">NMC-PT-2018-XXXX</span> · <span className="chip !bg-pine !text-white">Verified</span></div>
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full mt-1 px-3 py-2.5 rounded-xl border border-border bg-white focus:outline-none focus:ring-2 focus:ring-pine" />
    </div>
  );
}
