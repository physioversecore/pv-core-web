...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const add: CartCtx["add"] = async (item) => {
    try {
      const res = await CartActions.addToCart({
  ...
  const setQty = async (itemId: string, qty: number) => {
    ...
    try {
      await CartActions.updateCartItem(itemId, { quantity: Math.max(1, qty) });
    } catch {
      setItems(prev);
    }
  };
  ...
  const setRental = async (itemId: string, days: number) => {
    ...
    try {
      await CartActions.updateCartItem(itemId, { rentalDays: days });
    } catch {
      setItems(prev);
    }
  };
