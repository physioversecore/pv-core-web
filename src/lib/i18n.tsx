...
export function LangSwitcher({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <div className={`inline-flex items-center gap-0.5 p-0.5 rounded-full border ${dark ? "border-white/20 bg-white/10" : "border-border bg-white"}`}>
      <button onClick={() => setLang("en")} className={`${base} ${lang === "en" ? (dark ? "bg-white text-pine" : "bg-pine text-white") : dark ? "text-white/70" : "text-slate"}`}>EN</button>
      <button onClick={() => setLang("ne")} className={`${base} ${lang === "ne" ? (dark ? "bg-white text-pine" : "bg-pine text-white") : dark ? "text-white/70" : "text-slate"}`}>नेपा</button>
    </div>
  );
}
