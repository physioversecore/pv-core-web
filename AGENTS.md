...
- **Editorial design system** (Warm Ivory canvas + Muted Terracotta accent, Deep Forest text on ivory — pillow radii, hairline `#e5e2da` borders only, no drop shadows)
...
- **No hardcoded hex colors in code** — component colors must reference tokens only (Tailwind utilities like `bg-secondary`/`text-text`, or `var(--color-*)` inline). Raw hexes exist only in `globals.css` token definitions + `design-tokens.ts`, recharts `fill`/`tick` props may use `var(--color-*)`, and `GoogleIcon.tsx` keeps Google's trademark colors. Inline-CSS fallback pages (`utils/error-page.ts`) are exempt (standalone, cannot use app vars).
- Brand tokens (`globals.css` `@theme` → Tailwind utilities): `voltage-lime` (#d77c5f Terracotta), `cyan-spark` (#c4d3c2 Mineral Sage), `mid-abyss` (#16443B Deep Forest), `carbon-ink` (#0d2925 Evergreen Ink), `pure-white` (#ffffff), `true-black` (#000000), `ash` (#68736f Stone), `hairline` (#e5e2da)
- Dark-canvas tokens: `abyss-soft` (#2a5047), `abyss-mid` (#123b34), `abyss-deep` (#081411) — the landing page wraps `HeroSection` + `ServicesSection` in `.home-background`: one shared continuous atmosphere (vh-anchored forest-charcoal vertical base `abyss-soft → abyss-mid → abyss-deep → mid-abyss` + a terracotta radial glow centered on hero content fading out ~110–120vh past the hero boundary). Both sections are transparent; no seam, no clipped glow
- Dark-section text hierarchy: `ink-soft` (#edede9), `ink-muted` (#a8aaa4), `ink-faint` (#858587), `ink-dim` (#bcbdb7)
- Neutral/functional tokens (`@theme inline`): `text` (#1B2523), `text-light` (#4c5a55), `text-muted` (#68736f), `surface` (#ECEFE9), `background` (#F7F5EF), `card`/`popover` (#ffffff), `border` (#e5e2da), `input` (#d5d0c5), `ring` (#D77C5F), `primary` (#D77C5F), `secondary` (#16443B), `success` (#16a34a), `warning` (#f59e0b), `danger` (#C84B4B), `danger-bg` (#FBEAE5), `danger-ink` (#8A3018), `warn-bg` (#FCF3E1), `warn-ink` (#8A5A0B), `warn-border` (#ECD09B), `slot-*` (availability), `session-*` (schedule statuses: `session-reschedule`, `session-decline`, `session-completed`, `session-confirmed-bg`, `session-open-bg`, `session-past-bg`, `session-tip`), `surface-hover`, `slot-blocked`, `slot-off-border`, feature gradient tints `feature-forest`/`feature-amber`/`feature-rose`/`feature-rose-deep`
- Fonts (loaded via Google Fonts `<link>` in `layout.tsx`): **Manrope is the only primary typeface** (`font-sans`, `font-display`, `font-mono`, `font-anybody` all resolve to Manrope). Weights: 400 body, 500 supporting/labels/metadata, 600 nav/section labels/buttons/therapist names, 700 prices/important values, 800 hero + major headings. `body` uses `font-feature-settings: "tnum"`;
...
- Legacy brutalist class names kept but re-skinned: `btn-volt` (terracotta solid), `btn-carbon` (carbon-ink solid), `btn-outline-ink` (ghost 1px ink border), `card-neo` (white + hairline + 24px), `chip-volt/mint/sand` (pills), `input-neo` (24px radius), `label-ink` (sans medium), `grid-bg` (subtle 32px grid)
- Utility classes in `globals.css`: `btn-primary`, `btn-secondary`, `btn-outline`, `card-soft`, `chip`, `eyebrow`, `badge-*`, `tabs-filter`, `stats-grid`, `table-header`, `hatch-past`, `hatch-blocked`, `home-background` (shared hero+services dark canvas w/ terracotta glow, content z-index 1)
...
- Landing order: `HeroSection` → i18n heading (`landing.featuredTherapistsEyebrow`/`Title`) + `FeatureCarousel` → `ServicesSection` (all inside `.home-background`; services = premium dark "opportunity grid", 4-col, terracotta CTA → `/services`) → `PartnersMarquee` → `HowItWorksSection` → `TherapistCTA`
